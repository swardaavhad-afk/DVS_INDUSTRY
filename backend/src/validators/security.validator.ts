...
const alertTypeEnum = z.enum(['SAFETY', 'SECURITY', 'MAINTENANCE', 'FIRE', 'INTRUSION', 'ENVIRONMENTAL', 'CUSTOM']);
...
export const incidentQuerySchema = z.object({
  ...
  type:         z.enum(['FIRE','THEFT','INJURY','PROPERTY_DAMAGE','UNAUTHORIZED_ACCESS',
                        'EQUIPMENT_FAILURE','CHEMICAL_SPILL','NEAR_MISS','OTHER','all'])
  ...
  sortBy:       z.enum(['occurredAt','createdAt','severity','status']).optional().default('occurredAt'),
...
export const alertQuerySchema = z.object({
  ...
  pageSize:     z.coerce.number().int().positive().max(100).optional().default(20),
  search:       z.string().max(100).trim().optional(),
  type:         z.enum(['SAFETY','SECURITY','MAINTENANCE','FIRE','INTRUSION','ENVIRONMENTAL','CUSTOM','all'])
                  .optional().default('all'),
  severity:     z.enum(['LOW','MEDIUM','HIGH','CRITICAL','all']).optional().default('all'),
  status:       z.enum(['ACTIVE','ACKNOWLEDGED','RESOLVED','EXPIRED','all']).optional().default('all'),
  departmentId: z.coerce.number().int().positive().optional(),
